...
// // ==================================================
// // EXPORT ALL CHART INSIGHTS TO EXCEL
// // Add this entire block at the bottom of app.js
// // ==================================================
...
//     /*
//     Place the button in the dashboard header.
//     If the header is not found, place it at the
...
// // ==================================================
// // CONVERT A CHART INTO EXCEL ROWS
// // ==================================================

// function chartToRows(chartId) {
...
//                     /*
//                     Include report counts when the
//                     dataset contains reasonCounts.
//                     */

//                     if (
//                         Array.isArray(
//                             dataset.reasonCounts
//                         )
//                     ) {
...
//         console.warn(
//             `${sheetName} was not exported because no chart data was found.`
//         );
...
// async function exportDashboardInsights() {
...
//         /*
//         Check whether at least one worksheet
...
//             throw new Error(
//                 "No chart data was available to export."
//             );
//         }
...
//         XLSX.writeFile(
//             workbook,
//             `Report_Dashboard_Insights_${currentDate}.xlsx`
//         );
...
//         alert(
//             "The Excel file could not be created. Check the browser console for details."
//         );
...
function exportDashboardInsights() {
    ...
    console.log(
        Chart.getChart("regionChart")
    );

    console.log(
        Chart.getChart("reasonChart")
    );
        ...
        const rows =
            monthlyChart.data.labels.map(
                (label, index) => {
                    ...
                    monthlyChart.data.datasets.forEach(
                        dataset => {
    ...
    // Regional Compliance
    const regionChartExport =
        Chart.getChart(
            "regionChart"
        );

    if (regionChartExport) {

        const rows =
            regionChartExport.data.labels.map(
                (
                    region,
                    index
                ) => ({
    ...
    // Delay Reasons
    const reasonChartExport =
        Chart.getChart(
            "reasonChart"
        );

    if (reasonChartExport) {

        const rows =
            reasonChartExport.data.labels.map(
                (
                    reason,
                    index
                ) => ({
    ...
    // Monthly Outcome Breakdown

    const monthlyStackedChartExport =
        Chart.getChart(
            "monthlyStackedChart"
        );

    if (monthlyStackedChartExport) {

        const rows =
            monthlyStackedChartExport
                .data
                .labels
                .map(
                    (month, index) => {
